    ...
    if (contactForm) {
        ...
        const validators = {
            ...
            phone: (value) => {
                ...
                return phoneRegex.test(value.trim()) ? null : 'Please enter a valid phone number';
            },
            minlength: (value, min) => {
                if (!value.trim()) return 'This field is required';
                return value.trim().length >= min ? null : `Minimum ${min} characters required`;
            }
        };
        ...
        const validateField = (input) => {
            ...
            for (const rule of rules) {
                ...
                if (trimmedRule === 'email') {
                    const error = validators.email(input.value);
                    if (error) {
                        showError(input, error);
                        return false;
                    }
                } else if (trimmedRule === 'phone') {
                    const error = validators.phone(input.value);
                    if (error) {
                        showError(input, error);
                        return false;
                    }
                } else if (trimmedRule.startsWith('minlength:')) {
                    const min = parseInt(trimmedRule.split(':')[1], 10);
                    const error = validators.minlength(input.value, min);
                    if (error) {
                        showError(input, error);
                        return false;
                    }
        ...
        contactForm.addEventListener('submit', function(e) {
            ...
            let isValid = true;
            const firstError = { element: null, position: Infinity };
            ...
            inputs.forEach(input => {
                ...
                if (!fieldValid) {
                    ...
                    if (rect.top < firstError.position) {
                        firstError.element = input;
                        firstError.position = rect.top;
                    }
                }
            });
            
            if (!isValid && firstError.element) {
            ...
            if (isValid) {
                ...
                fetch(formAction, fetchOptions)
                    ...
                    .catch(error => {
                        const errorMessage = contactForm.querySelector('.form-error-global') || document.createElement('div');
...
(function(){
  try {
    ...
    document.addEventListener('submit', function(e) {
      ...
      var wid = '8c916101-134d-48d8-9d8b-7ec053be4873';
